...
* ###### <a id="csv">CSV, TSV</a>
    ...
    **[2]** One million rows is here suggested because many spreadsheet software tools, e.g. MS Excel, have limitations in opening CSV files bigger than this. However, CSV files with several million rows exist and can be managed using different software tools (e.g., a text editor). The CSV data format, however, is not best suited for very big datasets for which formats like parquet, described below, are a better choice.
